...
- In order to do multiple languages, you need to pass `i18n` (`i18n` should be configurated in the app level) into `CompareDetailComponent`, `RecommandBanner`, `SwitchStatusComponent`, as a root props. And then, you have to copy and paste all attributes of `product_comparison_component` in [texts](src/product-comparison-data.json) into your app locale file. You can also change text value, but DON'T change the key.
